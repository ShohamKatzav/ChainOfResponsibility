...
{
    internal class Electrician : CarHandler
    {
        public override void HandleRequest(Car car)
        {
            ...
            if (result < 6)
            {
                Console.WriteLine($"Test failed!! (Test result: {result}) Problem detected by: Electrician");
                car.isFixed = true;
                car.fixedHandlerName = GetType().Name;
            }
            else
            {
                Console.WriteLine($"Test passed!! (Test result: {result}) Diagnosed by: Electrician");
                if (NextHandler != null)
